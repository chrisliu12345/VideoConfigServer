package com.gd.dao.channel;

import com.gd.controller.channel.ChannelController;
...
public interface IChannelDao {
    ...
    @Select("<script> SELECT PlayUrl FROM tbl_channel where CamID=#{id}</script>")
    List<String> queryForPlayUrls(int id);

    @Insert("<script>INSERT INTO tbl_decoder_channel (ProtocolType,DecoderID,DecoderChannelID,CamID,MatrixID,MatrixInChannelNo) " +
            "VALUES (#{ProtocolType},#{DecoderID},#{DecoderChannelID},#{CamID},#{MatrixID},#{MatrixInChannelNo}" +
            ")</script>")
    void addEncoderChannelForObject(EncoderChannel encoderChannel);
    @Select("<script>SELECT MAX(windowcount) from tbl_windows_mode</script>")
    int queryForwindowsmode();
    @Select("<script>SELECT channelID from tbl_decoder_channel where DecoderID=#{CameraId}</script>")
    List<Integer> queryForEncoderChannelId(int CameraId);
    @Insert("<script>INSERT INTO tbl_win_to_channel (channel_id,win_seq) " +
            "VALUES (#{channel_id},#{win_seq}" +
            ")</script>")
